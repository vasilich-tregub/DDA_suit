...
void dt(int width, int height, std::vector<int>& distance)
{
    ...
    std::vector<int> vpos(max(width, height));         // Indices of locations of visible parabola vertices (lowest values), ordered by q 
    std::vector<int> vneg(max(width, height));
    std::vector<double> zpos(max(width, height) + 1);  // Indices of locations of boundaries, ordered by increasing q 
    std::vector<double> zneg(max(width, height) + 1);
